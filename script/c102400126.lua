...
function s.initial_effect(c)
	--1 "Blitzkrieg" monster + 1 "Blitzkrieg" monster with lower ATK (max. 400) / Once per turn (Quick Effect): You can destroy this card, and if you do, negate the next attack this turn from a monster your opponent controls, also Special Summon 1 Level 5 or lower "Blitzkrieg" monster from your hand or GY. Unaffected by card effects that target it and cannot be targeted for attacks, while you control a Level 5 or lower "Blitzkrieg" monster. Each time another "Blitzkrieg" monster is destroyed, place 1 Lightning Counter on this card for each of those cards. Destroy this card if there are 5 or more Lightning Counters on the field. If destroyed this way: Destroy all monsters your opponent controls.
	c:EnableReviveLimit()
...
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and Duel.Destroy(c,REASON_EFFECT)~=0 then
		if Duel.GetAttacker() then Duel.NegateAttack()
			...
			e1:SetCountLimit(1)
			e1:SetOperation(function() if Duel.GetTurnPlayer()~=tp then Duel.Hint(HINT_CARD,0,id) Duel.NegateAttack() end end)
...
function s.acop(e,tp,eg,ep,ev,re,r,rp)
	local ct=eg:FilterCount(s.filter,e:GetHandler(),tp)
