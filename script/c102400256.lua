...
function s.initial_effect(c)
	--During damage calculation, if your other monster battles while this card is in your hand or field (Quick Effect): You can Special Summon 1 "Armarine" monster from your Deck with a different name from the monster you control and/or in your GY, then destroy 1 card you control or in your hand. You can only use this effect of "Armarine Rider of Seahorses" once per turn. If this card is destroyed by a card effect: You can shuffle monsters from your GY into the Deck, including at least 1 "Armarine" monster, also, after that, Special Summon 1 "Armarine" Synchro Monster from your Extra Deck whose Level exactly equals the total Levels of those monsters. (This is treated as a Synchro Summon.)
...
function s.filter(c,e,tp)
	return c:IsSetCard(0xa6c) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and not c:IsCode(id)
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SpecialSummon(Duel.SelectMatchingCard(tp,s.filter,tp,LOCATION_HAND,0,1,1,nil,e,tp),0,tp,tp,false,false,POS_FACEUP)<1 then return end
