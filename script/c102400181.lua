...
function s.initial_effect(c)
	--2 LIGHT Warrior monsters The total Levels of this card's Fusion Materials must equal 7 or more. If this card is Fusion Summoned: You can target 1 face-up monster on the field; negate that target's effects until the end of this turn. You can only use this effect of "Fated Hero Kanaris" once per turn. Once per turn: You can target 1 monster your opponent controls; destroy it.
	c:EnableReviveLimit()
	...
	e1:SetCondition(function() return c:IsSummonType(SUMMON_TYPE_FUSION) end)
...
function s.mfilter(c,fc,sub,mg,sg)
	return c:IsFusionAttribute(ATTRIBUTE_LIGHT) and c:IsRace(RACE_WARRIOR) and (not sg or #(sg-c)==0 or sg:GetSum(Card.GetLevel)>6)
end
