...
function s.initial_effect(c)
	--If you control a LIGHT monster, except "Synth Hadouken": You can Special Summon this card from your hand. During your Main Phase: You can have your opponent shuffle their Deck, then each player excavates the bottom 3 cards of their opponent's Deck, and if they do, they take 1 excavated monster, and either banish it or send it to the GY, also, you place the rest on top of the Decks in any order. You can only use each effect of "Synth Hadouken" once per turn.
...
function s.xfilter(c)
	return c:IsSetCard(0x1d10) and c:IsType(TYPE_MONSTER)
end
...
function s.sptop(e,tp,eg,ep,ev,re,r,rp)
	...
	e0:SetTarget(aux.TargetBoolFunction(aux.NOT(Card.IsSetCard),0x1d10))
	...
	if sc then
		...
		local e5=e4:Clone()
		e4:SetOperation(function() for _,ef in ipairs{e0,e1,e2,e3,e4,e5} do if ef then ef:Reset() end end end)
		e5:SetCode(EVENT_CHAIN_SOLVED)
		e5:SetOperation(function() for _,ef in ipairs{e0,e1,e2,e3,e4,e5} do if ef then ef:Reset() end end end)
...
function s.mattg(e,c)
	return c:IsSetCard(0x1d10) and c:IsLocation(LOCATION_HAND) or c==e:GetLabelObject()
end
