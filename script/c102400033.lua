...
function s.initial_effect(c)
	--When your opponent activates a card or effect while your Deck has an even number of cards: Negate the effect, and if you do, place both that card and 1 LIGHT/Spell/Trap "uken" card (except "Hadoukenihilism") from your hand or field on the bottom of the Deck. You can only use 1 "Hadoukenihilism" per turn.
...
function s.filter(c)
	return c:IsFaceupEx() and c:IsSetCard(0x1d10) and c:IsAbleToDeck() and not c:IsCode(id)
end
...
function s.settg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsSetCard,tp,LOCATION_DECK,0,1,nil,0x1d10)
		and Duel.GetFieldGroupCount(tp,LOCATION_DECK,0)>=ct end
end
function s.sfilter(c,e,tp)
	if not c:IsSetCard(0x1d10) or c:IsCode(id) then return end
